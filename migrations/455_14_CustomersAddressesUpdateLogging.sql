...
add column RuleGroup int(10) unsigned
;

DROP TRIGGER IF EXISTS Customers.AddressLogDelete; 
CREATE DEFINER = RootDBMS@127.0.0.1 TRIGGER Customers.AddressLogDelete AFTER DELETE ON Customers.Addresses
FOR EACH ROW BEGIN
	INSERT 
	...
	SET LogTime = now(),
		...
		RuleGroup = OLD.RuleGroup;
END;

DROP TRIGGER IF EXISTS Customers.AddressLogUpdate; 
CREATE DEFINER = RootDBMS@127.0.0.1 TRIGGER Customers.AddressLogUpdate AFTER UPDATE ON Customers.Addresses
FOR EACH ROW BEGIN
	INSERT 
	INTO `logs`.AddressLogs
	SET LogTime = now(),
		...
		RuleGroup = NULLIF(NEW.RuleGroup, OLD.RuleGroup);
END;

DROP TRIGGER IF EXISTS Customers.AddressLogInsert; 
CREATE DEFINER = RootDBMS@127.0.0.1 TRIGGER Customers.AddressLogInsert AFTER INSERT ON Customers.Addresses
FOR EACH ROW BEGIN
	INSERT 
	INTO `logs`.AddressLogs
	SET LogTime = now(),

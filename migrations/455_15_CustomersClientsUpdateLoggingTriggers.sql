DROP TRIGGER IF EXISTS Customers.ClientLogDelete; 
CREATE DEFINER = RootDBMS@127.0.0.1 TRIGGER Customers.ClientLogDelete AFTER DELETE ON Customers.Clients
FOR EACH ROW BEGIN
	INSERT 
...
DROP TRIGGER IF EXISTS Customers.ClientLogUpdate; 
CREATE DEFINER = RootDBMS@127.0.0.1 TRIGGER Customers.ClientLogUpdate AFTER UPDATE ON Customers.Clients
FOR EACH ROW BEGIN
	INSERT 
	INTO `logs`.ClientLogs
	SET LogTime = now(),
...
DROP TRIGGER IF EXISTS Customers.ClientLogInsert; 
CREATE DEFINER = RootDBMS@127.0.0.1 TRIGGER Customers.ClientLogInsert AFTER INSERT ON Customers.Clients
FOR EACH ROW BEGIN
	INSERT 
	INTO `logs`.ClientLogs
	SET LogTime = now(),

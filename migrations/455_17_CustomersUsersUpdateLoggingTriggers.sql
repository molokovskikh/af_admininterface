DROP TRIGGER IF EXISTS Customers.UserLogDelete; 
CREATE DEFINER = RootDBMS@127.0.0.1 TRIGGER Customers.UserLogDelete AFTER DELETE ON Customers.Users
FOR EACH ROW BEGIN
	INSERT 
	INTO `logs`.UserLogs
	SET LogTime = now(),
...
DROP TRIGGER IF EXISTS Customers.UserLogUpdate; 
CREATE DEFINER = RootDBMS@127.0.0.1 TRIGGER Customers.UserLogUpdate AFTER UPDATE ON Customers.Users
FOR EACH ROW BEGIN
	INSERT 
	INTO `logs`.UserLogs
	SET LogTime = now(),
...
DROP TRIGGER IF EXISTS Customers.UserLogInsert; 
CREATE DEFINER = RootDBMS@127.0.0.1 TRIGGER Customers.UserLogInsert AFTER INSERT ON Customers.Users
FOR EACH ROW BEGIN
	INSERT 
	INTO `logs`.UserLogs
	SET LogTime = now(),

...
FROM     (Customers.Clients AS cd ,
  ...
  left join Customers.Intersection i ON i.PriceId  = oh.PriceCode AND i.RegionId = oh.regionCode AND i.ClientId = oh.ClientCode and i.LegalEntityId = a.LegalEntityId
  ...
  join Customers.Intersection ifu on ifu.ClientId=oh.ClientCode and ifu.PriceId  = oh.PriceCode AND ifu.RegionId = oh.regionCode
  left join usersettings.pricescosts pc ON pc.costcode = i.CostId
  left join Customers.OrderSwap os on os.ClientId = oh.ClientCode and os.SupplierId = s.Id

DROP PROCEDURE Reports.GetPayerCode;
...
begin
  ...
  if (inID is not null) then
    ...
    where
          p.PayerId = inID
        and (exists(select * from Customers.Clients cl inner join billing.PayerClients pc on cl.Id = pc.ClientId where pc.PayerId = p.PayerId))
    ...
    if ((inFilter is not null) and (length(inFilter) > 0)) then
      ...
      from
        billing.payers p
      where
        (exists(select * from Customers.Clients cl inner join billing.PayerClients pc on cl.Id = pc.ClientId where pc.PayerId = p.PayerId))
      order by p.ShortName;
    end if;
  end if;
end;

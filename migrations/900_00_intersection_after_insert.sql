	...
	IF @Skip IS NULL OR @Skip <> 1 THEN
		...
		WHERE
			c.Id = NEW.ClientId AND up.UserId IS NULL and (rcs.IgnoreNewPriceForUser = 0
				or exists(
					select *
					from Customers.UserPrices e
					where e.UserId = u.Id
						and e.RegionId = NEW.RegionId
						and e.PriceId = (
							select max(p.PriceCode)
							from Usersettings.PricesData c
								join Usersettings.PricesData p on c.FirmCode = p.FirmCode
							where c.PriceCode = NEW.PriceId and p.PriceCode <> c.PriceCode
						)
					)
			);

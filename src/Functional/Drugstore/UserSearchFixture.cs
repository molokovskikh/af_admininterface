...
namespace Functional
{
	...
	{
		private void TestSearchResultsByUserInfo(Browser browser, string columnName, string searchBy)
		{
			var sql = String.Format(@"select max({0}) from future.Users", columnName);
			TestSearchResults(browser, columnName, searchBy, sql);
		}

		private void TestSearchResultsByClientInfo(Browser browser, string columnName, string searchBy)
		{
			var sql = String.Format(@"select max({0}) from future.Clients", columnName);
			TestSearchResults(browser, columnName, searchBy, sql);
		}

		private void TestSearchResults(Browser browser, string columnName, string searchBy, string sql)
		{
		...
		private void AssetSearch(Browser browser, string searchBy, string text)
		{
		...
		public void SearchByJuridicalName()
		{
			...
			AssetSearch(browser, "SearchByJuridicalName", name);
		}
		...
		public void SearchByPayerId()
		{
			...
			AssetSearch(browser, "SearchByPayerId", payer.Id.ToString());
		}
		...
		public void Autosearch_by_client_name()
		{
			...
			browser.TextField(Find.ById("SearchText")).TypeText(client.Name);
			browser.Button(Find.ByValue("Поиск")).Click();
		...
		public void Autosearch_by_client_id()
		{
			...
			browser.TextField(Find.ById("SearchText")).TypeText(client.Id.ToString());
			browser.Button(Find.ByValue("Поиск")).Click();
		...
		public void Search_with_number_symbol()
		{
			...
			browser.TextField(Find.ById("SearchText")).TypeText(client.Users[0].Id.ToString());
			browser.Button(Find.ByValue("Поиск")).Click();
